...
impl <'conn,R> Query<'conn,R> where R: DescriptorsProvider + FromResultSet {
    pub(crate) fn new<'s>(conn: &'conn Connection, sql:  &'s str) -> Result<Query<'conn,R>, oci::OracleError> {
        let stmthp = oci::stmt_prepare(conn.svchp, conn.errhp, sql)?;
        // execute statement for explicit describe

        // let errhp = conn.errhp;
        // oci::stmt_exec_for_describe(conn.svchp, stmthp, errhp)?;

        // get the number of columns in statement (may be query)
        /*
        let mut num_columns:u32 = 0;
        let num_columns_ptr: *mut u32 = &mut num_columns;
        oci::attr_get(stmthp as *mut oci::c_void, oci::OCI_HTYPE_STMT, num_columns_ptr as *mut oci::c_void, oci::OCI_ATTR_PARAM_COUNT,errhp)?;

        if num_columns > 0 {
            println!("Statement num_columns: {}", num_columns);
            // iterate throw 1 .. num_columns exclusive
            for idx in 1..=num_columns {
                let dtype = oci::get_column_dtype(stmthp, idx, errhp)?;
                println!("col {} type: {:?}", idx, dtype)
            }
        }
        */

        Ok( Query { conn, stmthp, fetcher: None, _result: PhantomData } )
    }
